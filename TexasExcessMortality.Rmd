...
tmp.mod <- tibble(week_ending_date = tmp$week_ending_date[4:nrow(tmp)],
                  ...
                  label = rep('real', length(resid.tmp.var[,'Texas'])))
...
Finally, I narrow down to the largest four states and the start of 2021 to get a sense for the deviations that are possible in large states. Unfortunately, selecting neighboring state to Texas is probably not the right approach. Ultimately, given that the presentation is in absolute deaths in excess of expectations the analysis could end up being biased by the size of the state. Larger states would be expected to have larger numbers of deaths incorrectly categorized.

That being said, there probably is some signal there in the final two weeks of February 2021 in Texas during the Winter Storm.
